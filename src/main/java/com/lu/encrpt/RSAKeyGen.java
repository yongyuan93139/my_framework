...
public class RSAKeyGen
{
    ...
    public static void main(String[] args) throws Exception{
		RSAKeyGen.init();
		KeyPair kp = RSAKeyGen.generateKey();
		PublicKey pubkey = kp.getPublic();
		PrivateKey privateKey = kp.getPrivate();
		String pubkeyStr = RSAKeyGen.getKeyAsString(pubkey);
		String privateKeyStr = RSAKeyGen.getKeyAsString(privateKey);
...
//		pubkeyStr = "-----BEGIN PUBLIC KEY-----MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCmJFvYWGh0kVkPusr1iOXoqOtttnTiNOT+ENdxM4eof+rC01MnHgG8QbzYtgrCJrTUI0vPPMqkdCnHQqE8D4LiZWN36KWgTvzPmUEH7dLVoc5EN3EvcgCv9LUQCiesniu+/0QtUnqkQNR3nQJBi+ePJKqcq0mtwXwKtKRCs/+YHQIDAQAB-----END PUBLIC KEY-----";
		...
		try
		{
			File privateKeyFile = new File("d:/rsa_private_key.pem");
			File publicKeyFile = new File("d:/rsa_public_key.pem");
			FileUtils.writeByteArrayToFile(privateKeyFile, privateKeyStr.getBytes());
			FileUtils.writeByteArrayToFile(publicKeyFile, pubkeyStr.getBytes());
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	private static void extractDb() throws NoSuchAlgorithmException, ClassNotFoundException, SQLException, Exception
	{
		...
		String pu2 = RSAKeyGen.getKeyAsString(pu);
		String pr2 = RSAKeyGen.getKeyAsString(pr);
		...
		else
		{
			...
			String pt = "12345";
			//公钥加密，私钥解密
			String ct = RSAKeyGen.encrypt(pt, pu);
			String pt2 = RSAKeyGen.decrypt(ct, pr);
			System.out.println("plaText=" + pt);
			System.out.println("cipText=" + ct);
			System.out.println("plaText2=" + pt2);

			//私钥加密，公钥解密
			String ctt = RSAKeyGen.encrypt(pt, pr);
			String ptt2 = RSAKeyGen.decrypt(ctt, pu);
